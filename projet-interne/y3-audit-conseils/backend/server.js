...
const nodemailer = require('nodemailer');
const cors = require('cors');
const multer = require('multer'); // <-- pour gérer les fichiers joints
require('dotenv').config();
...
// ✅ Route pour envoyer un email simple (prise de rendez-vous)
app.post('/send-email', async (req, res) => {
  const { to, subject, text, html } = req.body;
  ...
  try {
    const info = await transporter.sendMail({
      ...
      to,
      cc: process.env.SMTP_USER, // ✅ copie de sécurité
      subject,
      text,
      html,
    });
...
app.post('/send-email-with-attachment', upload.single('cv'), async (req, res) => {
  try {
    ...
    // ✅ Si un fichier (CV) est joint, l’ajouter à l’email
    if (req.file) {
      mailOptions.attachments.push({
        filename: req.file.originalname,
        content: req.file.buffer,
      });
      console.log(`📎 Fichier joint détecté : ${req.file.originalname}`);
    }

    const info = await transporter.sendMail(mailOptions);
    console.log('📨 Candidature envoyée avec succès :', info.messageId);
